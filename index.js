...
const USER_BOTOMETER_PATH =
  "https://botometer-pro.p.rapidapi.com/4/check_account";
const BEARER_TOKEN =
  "AAAAAAAAAAAAAAAAAAAAACCiZgEAAAAAHiLrAtS4SoUfnKoCSJ045B0JwqA%3DbftuB6RiaA4i2yFj3GT5KJ3bqWcYtBXjmTXyJo8woi50SAwuhB";
...
const getUserMentionsData = async (userId) => {
  ...
  const mentionsData = await axios
    .request(options)
    .then(function (mentionsRes) {
...
app.get("/getUserBotometer", async (req, res) => {
  ...
  if (
    ...
    const customMentionsPayload = {
      ...
      coordinates: null,
      created_at:
        (mensionData[0]?.created_at &&
          moment(mensionData[0]?.created_at).format(
      ...
      entities: {
        ...
        user_mentions:
          (mensionData.length &&
            mensionData[0]?.entities &&
            mensionData[0]?.entities?.mentions &&
            mensionData[0]?.entities?.mentions.length &&
            mensionData[0]?.entities?.mentions.map((data) => {
              return {
                id: +data.id,
                id_str: data.id,
    ...
    const customTimelinePayload = {
      ...
      created_at:
        (twitterData[0]?.created_at &&
          moment(twitterData[0]?.created_at).format(
            "ddd MMM DD HH:mm:ss ZZ YYYY"
          )) ||
        null,
      entities: {
        ...
        user_mentions:
          (twitterData.length &&
            twitterData[0]?.entities &&
            twitterData[0]?.entities?.mentions &&
            twitterData[0]?.entities?.mentions.length &&
            twitterData[0]?.entities?.mentions.map((data) => {
              return {
                id: +data?.id,
                id_str: data?.id,
                indices: [data?.start, data?.end],
